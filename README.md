...
This repository used to be consumed as a `.recipes` git submodule with symlinks into `.claude/`. The plugin form replaces that. Until every project has migrated, the legacy path stays in place and keeps working, meaning `bin/install`, `settings.base.json`, `skills.manifest.json`, `templates/`, and the root `CLAUDE.md`. They are removed once the last project moves over. Third-party skills are still fetched with the `skills` CLI as listed in `skills.manifest.json`, and the project template takes that job over.
